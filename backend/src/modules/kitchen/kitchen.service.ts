...
export class KitchenService {
  ...
  async getKdsOrders(stationId?: string, course?: string) {
    ...
    return orders.map(order => {
      ...
      if (stationId) {
        filteredItems = filteredItems.filter(item => item.product.station?.id === stationId);
      }

      if (course) {
        filteredItems = filteredItems.filter(item => item.product.course === course);
      }
  ...
  async bumpOrder(orderId: string) {
    ...
    const promises = order.items
      ...
      .map(item => this.ordersService.updateOrderItemStatus(item.id, 'READY'));
