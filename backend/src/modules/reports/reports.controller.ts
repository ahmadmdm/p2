import { Controller, Get, UseGuards, Query, Res, StreamableFile } from '@nestjs/common';
...
export class ReportsController {
  ...
  @Get('export/excel')
  async exportExcel(@Query('startDate') startDate: string, @Query('endDate') endDate: string, @Res({ passthrough: true }) res: Response) {
    const start = startDate ? new Date(startDate) : new Date(new Date().setDate(new Date().getDate() - 30)); // Default last 30 days
  ...
  @Get('export/pdf')
  async exportPdf(@Query('startDate') startDate: string, @Query('endDate') endDate: string, @Res({ passthrough: true }) res: Response) {
